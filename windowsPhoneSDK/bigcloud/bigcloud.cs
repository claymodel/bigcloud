...
// Please see http://support.bigcloud/kb/reference/bigcloud-server-api-reference

namespace bigcloud
{
    public class bigcloud
    {
        private static bigcloud sharedInstance;
        private Timer timer;
        ...
        private List<bigcloudEvent> eventqueue;

        private Timer EventTimer;

        public static bigcloud SharedInstance()
        {
            if (null == sharedInstance)
            {
                sharedInstance = new bigcloud();
            }
        ...
        private bigcloud()
        {
            ...
            eventqueue = new List<bigcloudEvent>();
        ...
        public void PostEvent(bigcloudEvent Event, int TimeoutTime = 30000)
        {
            if (Event == null)
            {
        ...
        public class bigcloudEvent
        {
            public bigcloudEvent()
            {
                Key = "";
                //UsingSum = false;
    ...
    public class ConnectionQueue
    {
...
#if (DEBUG)
                                Debug.WriteLine("bigcloud:\t" + "ok -> " + CurrentQuery);
#endif
                            }
                            catch (Exception E)
                            {
        ...
        public void QueueEvents(List<bigcloud.bigcloudEvent> Events)
        {
            string data = "";
            data += "app_key=" + AppKey;
            data += "&" + "device_id=" + DeviceInfo.getUDID();
            ...
            //bool First1 = true;
            //foreach(bigcloud.bigcloudEvent CurrentEvent in Events)
            //{
            //    if (First1)
            //    {
            //        First1 = false;
                ...
                thread = new Thread(new ParameterizedThreadStart((o) =>
                    {
                        ...
                        while (true)
                        {
                            if (queue.Count != 0)
                            {
                                    ...
                                    Request.BeginGetResponse(new AsyncCallback((Async) =>
                                        {
                                            try
                                            {
                                                ...
                                                Debug.WriteLine("bigcloud:\t" + "returned -> " + ReturnedString);
                                                Debug.WriteLine("bigcloud:\t" + "ok -> " + Data);
#endif
                                                queue.Dequeue();
                                            }
                                            catch (Exception E)
                                ...
                                {
#if (DEBUG)
                                    Debug.WriteLine("bigcloud:\t" + E.ToString());
                                    Debug.WriteLine("bigcloud:\t" + "error -> " + Data);
#endif
...
#if (DEBUG)
                thread.Name = "bigcloud Worker Thread";
#endif
                //StopThread = false;
...
//                                        {
//                                            Debug.WriteLine("bigcloud:\t" + (E as WebException).Message + "\t" + (E as WebException).Status);
//                                        }
//#endif
//                                    }
//                                    finally
...
//                            {
//#if (DEBUG)
//                                Debug.WriteLine("bigcloud:\t" + "Worker thread abort recieved and respected");
//#endif
//                                return;
//                            }
